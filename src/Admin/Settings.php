<?php

namespace WPGraphQLGutenberg\Admin;

use WPGraphQLGutenberg\Admin\Editor;
use WPGraphQLGutenberg\PostTypes\BlockEditorPreview;

class Settings {
	function __construct() {
		add_action('admin_menu', function () {
			add_menu_page(
				__('GraphQL Gutenberg', 'wp-graphql-gutenberg'),
				'GraphQL Gutenberg',
				'manage_options',
				'wp-graphql-gutenberg-admin',
				array($this, 'updateBlockRegistryPage'),
				'data:image/svg+xml;base64,'
			);
			add_submenu_page('wp-graphql-gutenberg-admin', __('GraphQL Gutenberg', 'wp-graphql-gutenberg'), 'Update registry', 'manage_options', 'wp-graphql-gutenberg-admin', array($this, 'updateBlockRegistryPage'));
			add_submenu_page('wp-graphql-gutenberg-admin', 'Mini Mode', 'Mini Mode', 'manage_options', 'wp-graphql-gutenberg-mini-mode', array($this, 'miniModePage'));
		});

		add_action('admin_enqueue_scripts', function ($hook) {
			if (!preg_match('/.+wp-graphql-gutenberg-admin$/', $hook)) {
				return;
			}

			wp_enqueue_style('wp-components');

			Editor::enqueue_script();

			wp_localize_script(Editor::$script_name, 'wpGraphqlGutenberg', [
				'adminPostType' => BlockEditorPreview::post_type(),
				'adminUrl' => get_admin_url()
			]);
		});

		add_action('admin_init', array($this, 'settings'));
	}

	function settings(){
		add_settings_section('wpgqlg_section', null, null, 'wp-graphql-gutenberg-mini-mode');
		add_settings_field('wpgqlg_minimode', 'Mini Mode', array($this, 'miniModeHTML'), 'wp-graphql-gutenberg-mini-mode', 'wpgqlg_section');
		register_setting('wpgqlgplugin', 'wpgqlg_minimode', array('sanitize_callback' => 'sanitize_text_field', 'default' => '0'));
	}

	function miniModeHTML(){ ?>
		<input type="checkbox" name="wpgqlg_minimode" <?php checked(get_option('wpgqlg_minimode'), 'on') ?>>
	<?php }

	function updateBlockRegistryPage(){
		echo '<div class="wrap"><div id="wp-graphql-gutenberg-admin"></div></div>';
	}

	function miniModePage(){ ?>
		<div class='wrap'>
			<h1>Mini Mode</h1>
			<h2>Enable this mode when working with Gatsby</h2>
			<?php settings_errors(); ?>
			<form action="options.php" method="POST">
				<?php
					settings_fields('wpgqlgplugin');
					do_settings_sections('wp-graphql-gutenberg-mini-mode');
					submit_button();
				?>
			</form>
		</div>
	<?php }
}