<?php

namespace WPGraphQLGutenberg\Admin;

use WPGraphQLGutenberg\Admin\Editor;
use WPGraphQLGutenberg\PostTypes\BlockEditorPreview;

class Settings {
	function __construct() {
		add_action('admin_menu', function () {
			add_menu_page(
				__('GraphQL Blocks', 'wp-graphql-gutenberg'),
				'GraphQL Blocks',
				'manage_options',
				'wp-graphql-blocks-admin',
				array($this, 'updateBlockRegistryPage'),
				'data:image/svg+xml;base64,'
			);
			add_submenu_page('wp-graphql-blocks-admin', __('GraphQL Blocks', 'wp-graphql-gutenberg'), 'Update registry', 'manage_options', 'wp-graphql-blocks-admin', array($this, 'updateBlockRegistryPage'));
		});

		add_action('admin_enqueue_scripts', function ($hook) {
			if (!preg_match('/.+wp-graphql-blocks-admin$/', $hook)) {
				return;
			}

			wp_enqueue_style('wp-components');

			Editor::enqueue_script();

			wp_localize_script(Editor::$script_name, 'wpGraphqlGutenberg', [
				'adminPostType' => BlockEditorPreview::post_type(),
				'adminUrl' => get_admin_url()
			]);
		});
	}

	function updateBlockRegistryPage(){
		echo '<div class="wrap"><div id="wp-graphql-blocks-admin"></div></div>';
	}
}